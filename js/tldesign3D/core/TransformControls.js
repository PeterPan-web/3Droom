	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		function onPointerMove( event ) {
			...
			if ( _mode === "translate" ) {
				...
				if ( scope.space === "local" ) 
				   {
				         var _scalex = 1+ point.x/100;
				         var _scaley = 1 + point.y/100;
				         var _scalez = 1 + point.z / 100;
				         if (point.x < 0 && point.x>=-100) {
				             _scalex = 1;
				         } else if (point.x < -100) {
				             _scalex = 1/point.x* (-100);
				         }
				         if (point.y < 0 && point.y >= -100) {
				             _scaley = 1;
				         } else if (point.y < -100) {
				             _scaley = 1 / point.y * (-100);
				         }
				         if (point.z < 0 && point.z >= -100) {
				             _scalez = 1;
				         } else if (point.z < -100) {
				             _scalez = 1 / point.z * (-100);
				         }
				        console.log("_scalex" + _scalex + "_scaley" + _scaley + "_scalez" + _scalez);
				        if (scope.axis === "XYZ") {

				            scale = 1 + ((point.y) / Math.max(oldScale.x, oldScale.y, oldScale.z));
				            scope.object.scale.x = oldScale.x*_scaley;
				            scope.object.scale.y = oldScale.y * _scaley;
				            scope.object.scale.z = oldScale.z * _scaley;

				        } else {

				            point.applyMatrix4(tempMatrix.getInverse(worldRotationMatrix));
				            if (scope.axis === "X") scope.object.scale.x = oldScale.x * _scalex;
				            if (scope.axis === "Y") scope.object.scale.y = oldScale.y * _scaley;
				            if (scope.axis === "Z") scope.object.scale.z = oldScale.z * _scalez;
				        }
				    }
